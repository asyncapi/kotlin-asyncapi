...
You can register multiple extensions to extend and override AsyncAPI components. Extensions with a higher order override extensions with a lower order. Please note that you can extend top-level components only for now (`info`, `channels`, `servers`...). Subcomponents will always be overwritten.
